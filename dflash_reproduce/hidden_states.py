...
# --------------------------------------------------------------------------- #
# Utility: Direct target model forward pass extractor
# --------------------------------------------------------------------------- #


class DirectExtractor(HiddenStatesExtractor):
    """Direct extractor that runs the target model in-process.

    This is the simplest extractor - it loads the target model and runs
    forward passes directly. Suitable for single-GPU training or when
    the target model fits in memory alongside the draft model.

    Unlike OfflineHiddenStatesExtractor, this does NOT cache results to disk.
    Each call runs a fresh forward pass.
    """
    ...
    def __init__(
        ...
        Args:
            target_model: The target model instance (already loaded).
            target_layer_ids: Layer IDs to extract hidden states from.
            offset: Offset for layer ID adaptation.
        """
        super().__init__(target_layer_ids)
        ...
        dummy_input = torch.zeros(1, 1, dtype=torch.long, device=next(target_model.parameters()).device)
        with torch.no_grad():
            outputs = target_model(dummy_input, output_hidden_states=True)
            sample_hidden = outputs.hidden_states[0]
            self.target_hidden_dim = sample_hidden.shape[-1]
            self._output_dim = self.num_layers * self.target_hidden_dim

    def extract(self, input_ids: Tensor) -> Tensor:
        """Extract hidden states by running the target model.
        ...
        # Use the extract_context_feature utility
        result = extract_context_feature(
            list(all_hidden_states), self.target_layer_ids, offset=self.offset
        )
