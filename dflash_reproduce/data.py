...
def compute_loss_weights(block_size: int, gamma: float) -> torch.Tensor:
    ...
    Matches the DFlash paper (Equation 4) with exponentially decaying
    weights that prioritise early positions in each block:

    .. math::
        w_k = \exp\left(-\frac{k - 1}{\gamma}\right)

    where :math:`k \in [1, \text{block_size}]` is the position within the
    block (1 = anchor, block_size = last masked position) and
    :math:`\gamma` controls the decay rate.

    Paper values: block_size=16 -> gamma=7, block_size=10 -> gamma=5,
    block_size=8 -> gamma=4.

    Args:
        block_size: Number of positions per block (including anchor).
        gamma: Decay hyper-parameter (smaller = faster decay).

    Returns:
        1-D float tensor of shape ``(block_size,)``.  Index ``i``
        corresponds to block position ``k = i + 1``.
    """
    k = torch.arange(1, block_size + 1, dtype=torch.float32)  # k = 1 .. block_size
...
def build_sparse_attention_mask(
    ...
    """Vectorised implementation of the DFlash sparse block-attention mask.
...
class DFlashTrainingDataset(Dataset):
    ...
    def _build_labels_and_weights(
        ...
        """Build labels and per-position weights for block-diffusion training.

        In DFlash block diffusion (paper Section 4.2, Figure 4):
        - The **anchor** position is a *clean* token (yellow); it is NOT
          masked and contributes no loss -> label ``-100``, weight 0.
        - The remaining ``block_size - 1`` positions are *mask tokens*
          (green).  The draft model must predict the **original** token at
          each masked position.  Label = ``input_ids[pos]`` (not
          ``input_ids[pos+1]`` as in autoregressive LM training).
        - Loss weight follows paper Equation 4:
          ``w_k = exp(-(k-1) / gamma)`` where k = 1..block_size is the
          position within the block (k=1 = anchor, k=2 = first mask, ...).

        Returns:
            Tuple of (labels, position_weights), both tensors of shape
            ``(seq_len,)``.  Non-mask positions have label ``-100`` and
            weight ``0``.
        """
        seq_len = len(input_ids)
        labels = torch.full_like(input_ids, -100)
        pos_weights = torch.zeros(seq_len, dtype=torch.float32)

        for anchor in anchor_positions:
            anchor_int = int(anchor.item())
            end = min(anchor_int + self.block_size, seq_len)
            # Iterate through the block: offset 0 = anchor, 1 = first mask, ...
            for offset in range(end - anchor_int):
                pos = anchor_int + offset
                k = offset + 1  # block position k = 1..block_size
                if pos >= seq_len:
                    break
                if offset == 0:
                    # Anchor (k=1): clean token, no loss
                    labels[pos] = -100
                    pos_weights[pos] = 0.0
                else:
                    # Mask position (k >= 2): predict original token
                    labels[pos] = input_ids[pos]
                    # Weight index = k-1 = offset; loss_weights[0] is anchor
                    if offset < len(self.loss_weights):
                        pos_weights[pos] = self.loss_weights[offset]
        return labels, pos_weights
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if len(anchor_positions) > 0:
            attention_mask = build_sparse_attention_mask(
        ...
        # Build labels and per-position weights
        labels, pos_weights = self._build_labels_and_weights(
        ...
        return {
            ...
            "labels": labels,
            "loss_weights": pos_weights,  # [seq_len] per-position weights
...
def dflash_collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for b_idx, example in enumerate(batch):
        ...
        loss_weights[b_idx, :seq_len] = example["loss_weights"]
