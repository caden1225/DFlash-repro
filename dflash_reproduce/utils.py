...
def setup_distributed(config) -> int:
    """Initialize PyTorch distributed training.

    Args:
        config: Distributed configuration (``DistributedConfig`` or compatible).

    Returns:
        Process rank (0 for single-process training).
    ...
    if not dist.is_initialized():
        init_method = getattr(config, "init_method", None) or "env://"
        try:
...
def load_target_model(
    ...
    Args:
        config: Model configuration (``dflash_reproduce.config.ModelConfig`` or
            the lightweight loader config defined in this module).
    ...
    model_name_or_path = getattr(config, "model_name_or_path", None) or getattr(
        config, "target_model", None
    )
    if not model_name_or_path:
        raise ValueError(
            "Model config must specify 'target_model' or 'model_name_or_path'"
        )

    trust_remote_code = getattr(config, "trust_remote_code", True)
    use_fast_tokenizer = getattr(config, "use_fast_tokenizer", True)
    cache_dir = getattr(config, "cache_dir", None)
    device = getattr(config, "device", None) or str(get_device())
    dtype = getattr(config, "dtype", "auto")

    logger = logging.getLogger("dflash")
    logger.info("Loading target model from %s", model_name_or_path)
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to load target model from {model_name_or_path}: {e}"
        ) from e
