...
class DFlashTrainer:
    ...
    def compute_loss(
        ...
        Only computes loss at positions where labels != -100 (the ignore
        index).  Applies DFlash position-dependent weights (Equation 4) if
        provided.

        The per-position weights are pre-computed in
        :meth:`DFlashTrainingDataset._build_labels_and_weights` so that
        anchor positions have weight 0 (and label -100) while mask
        positions have weight ``w_k = exp(-(k-1) / gamma)`` where
        ``k = 2 .. block_size``.
        ...
        Args:
            ...
            loss_weights: Optional per-position weights [batch, seq_len]
                or [seq_len].  Must match the shape of ``labels``.
        ...
        # Apply DFlash position-dependent weights
        if loss_weights is not None and loss_weights.numel() > 0:
            if loss_weights.dim() == 1 and loss_weights.shape[0] == seq_len:
                # [seq_len] -> expand to [batch, seq_len] -> flatten
                loss_weights = loss_weights.unsqueeze(0).expand(batch_size, -1)
            # loss_weights is now [batch, seq_len]
