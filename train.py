...
from dflash_reproduce.data import build_dataloader, build_target_tokenizer
...
def main() -> None:
    ...
    # ------------------------------------------------------------------ #
    rank = setup_distributed(config.distributed) if config.distributed else 0
    ...
    logger = setup_logging(str(log_dir), rank=rank)
    logger.info("=" * 70)
